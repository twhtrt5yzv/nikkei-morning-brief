...
毎朝の支社メンバー向け情報連携用アプリ。日経（nikkei.com）の記事見出しをGoogleニュース経由で自動収集し、カテゴリ別（けさの朝刊トピックス／トップ／株価／為替／金利／外貨・ドル建て／円建て・保険）に表示。朝刊トピックスは「当日0〜7時台に電子版配信された記事」＝朝刊掲載分の近似。記事を選んでコメントを付け、LINE・メールにそのまま貼れる連携文を生成できる。
