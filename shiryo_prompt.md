...
3. **反論を先回りして封じる。** 資料Aには経営者が必ず口にする疑問トップ3とその切り返しをQ&Aで載せる。切り返しは感情論でなく、事実か構造で返す。**切り返しの論理は同フォルダの playbook.md（シェアイン最大化戦略）から取ること。** 核となる2つの型:
   - 対 一時払い:「申込日のピンポイント為替で全額ロック＝高値掴みリスク。平準払いはドルコスト平均法で時間分散できる」
   - 対 変額保険:「勇退・承継は期日をずらせないイベント。退職の年に暴落したら勇退延期か退職金減額の二択になる。相場に左右されないコア枠が先」
   その他の反論（資金拘束・様子見・保険料負担等）も含め、その日のニュースに最も関係する3つを選ぶ。前日と同じ組み合わせは避ける。
